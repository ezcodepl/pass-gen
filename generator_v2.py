...
def save_to_file(file_type):
    ...
    default_filename = f"pass_{date.today().strftime('%Y-%m-%d')}"
...
min_requirements_label = tk.Label(frame, text="(min. 8 znaków)", bg="#008B8B", fg="white")
min_requirements_label.pack(side=tk.LEFT, padx=(0, 5))
...
progress_bar = ttk.Progressbar(strength_frame, orient="horizontal", length=300, mode="determinate",
                               style="WeakPassword.Horizontal.TProgressbar")
progress_bar.pack(side=tk.LEFT, padx=10)

# Rozpoczęcie działania głównej pętli programu
app.mainloop()
